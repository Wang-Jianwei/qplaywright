...
如果本草案落地，应直接替换 [docs/mcp_end_state.md](docs/mcp_end_state.md) 和 [docs/mcp_end_state_schema.md](docs/mcp_end_state_schema.md) 中与 handle identity、snapshot refs、widget discovery 相关的契约，而不是作为并行兼容层存在。
...
1. `widgets` 是面向机器的稳定 widget handle 列表，不再叫 `refs`。
