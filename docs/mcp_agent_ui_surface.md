...
本文中的 stable widget handle 指 MCP 对外暴露的 target identity。
...
文档示例里的 `target`、`owner`、`window.target` 都指 MCP external handle。
...
4. response `target` 是实际解析后的 snapshot root handle；即使 request `target=null`，response 也不应继续返回 `null`。
5. `window.target` 同样应是稳定 widget handle，不暴露内部整数 `wid`。
...
7. response `root` 返回实际解析后的 root handle；即使 request `root=null`，response 也不应继续返回 `null`。
...
1. `root` 是实际解析后的 root external handle。
