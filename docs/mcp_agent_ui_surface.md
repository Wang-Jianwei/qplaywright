...
3. 如果 handle 所属 window 隐藏、最小化、关闭或被不可交互模态窗口阻塞，动作失败并返回明确错误。
4. 如果动作成功导致 Qt active window 改变，服务端更新 active window tracking，并在响应中返回新的 `active_window`。
...
2. 其次使用 current text。
3. 再其次使用 value。
4. 不把 accessible name 混入 `text`；accessible name 只由 `accessible_name` 谓词匹配。
...
6. 同级节点按 widget tree preorder 排序。
7. 最后按 stable widget handle 排序，确保完全 deterministic。
...
2. 引入统一 target parser：selector 按 active window scope 解析，handle 按 session-global scope 解析。
3. 让所有 widget-oriented action 接受 stable widget handle。
4. 增加 handle lifetime 与 cross-window target scope tests。
