...
- `observation: "none"` 为默认值
- 当为 `"full_tree"` 或 `"screen_visible"` 时，服务端附带 post-action observation、`root_handle` 和 `tree`
- `observation="screen_visible"` 只适用于仍然保有 target scope 的动作后观察，不应在切窗后偷偷回退成 window-wide snapshot
...
- 它是服务端 discovery 的主入口，而不是让模型扫整棵树
- 它能承接“角色 + 文本 + 状态”这类复合约束，而不把 selector 语法扩成布尔表达式语言
...
职责：对 table/tree/list/tab owner widget 做结构化后代发现。
...
- `items[*].target` 应该是由 `inspect_items` 生成的 opaque itemref 字符串，供 `inspect`、`click`、`hover`、`wait`、`set_expanded` 直接复用

为什么保留：

- item-view logical structure 不应混入真实 widget discovery
- `find` 负责 QWidget discovery，`inspect_items` 负责 structured descendants
...
- `count=1` 表示单击
- `count=2` 表示双击
- 当按 active window 坐标点击时，使用空 `target` 加 `point="x,y"`
...
- `mode="condition"` 时必须提供 `condition` 和字符串 `expected`
- `timeout=0` 表示回退到当前 session timeout
- `observation!="none"` 时，等待成功后返回 post-wait observation、`root_handle` 和 `tree`
...
- 动作工具默认返回精简结构
- 动作工具统一支持 `observation`
- `observation` 默认 `"none"`
- 当 `observation="full_tree"` 或 `"screen_visible"` 时，返回值附带 post-action observation、`root_handle` 和 `tree`
...
4. `snapshot {"depth": 6}`
5. `click {"target": "w12", "observation": "full_tree"}`
6. `input {"target": "#amount_editor", "text": "123.45", "mode": "replace", "submit": false}`
7. `invoke {"target": "#amount_editor", "method": "setCurrency", "args_json": "{\"code\": \"CNY\"}"}`
8. `scroll {"target": "#result_table", "delta_y": 480, "observation": "screen_visible"}`
9. `wait {"target": "#status_label", "mode": "state", "state": "visible", "timeout": 5}`
...
- 为动作工具加入 `observation` 和 `window_changed` 返回约定
- 收拢 `select_option` 到 `choose`

### Phase 3

- 新增 `find` 作为一等 server-side discovery 工具
- 为 item-view discovery 明确 `inspect_items` 契约
- 删除所有 `browser_*` 工具
